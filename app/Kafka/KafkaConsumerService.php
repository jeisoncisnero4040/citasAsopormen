...
use RdKafka\Message;
use App\Exceptions\CustomExceptions\QueuException;
        ...
        while ($this->running) {
            ...
            switch ($message->err) {
                ...
                default:
                    ...
                    throw new QueuException($message->errstr(), $message->err);
            }
        }
    ...
    private function processMessage(Message $message): void
    {
        ...
        while ($retries < $this->maxRetries) {
            ...
            try {
                ...
                if (!$handler) {
                    throw new QueuException("No handler for event: $event");
                }
    ...
    private function decodePayload(Message $message): array
    {
        try {
            ...
            if (!isset($payload['event']) || !is_string($payload['event'])) {
                throw new QueuException('Payload inválido: falta event');
            }
